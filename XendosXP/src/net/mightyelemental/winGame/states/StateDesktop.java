...
public class StateDesktop extends BasicGameState {
	...
	public void init(GameContainer gc, StateBasedGame delta) throws SlickException {
		...
		for (Class<? extends AppWindow> c : XendosMain.programs.keySet()) {
			...
			guiComponents.add(new FileObject(15,
					15 + (OSSettings.FILE_DISPLAY_SIZE + 25) * i, FileType.Program, title)
	...
	public void mouseDragged(int oldx, int oldy, int newx, int newy) {
		...
		for (int i = windowList.size() - 1; i >= 0; i--) {
			...
			// TODO: check
			windowList.get(i).mouseDragged(oldx, oldy, newx, newy);
		...
		if (selectFlag) {
			if (newy - this.oldy < 0) {
				if (newx - this.oldx < 0) {
					selection = new Rectangle(newx, newy, -newx + this.oldx,
							-newy + this.oldy);
				} else {
					selection = new Rectangle(this.oldx, newy, newx - this.oldx,
							-newy + this.oldy);
				}
			} else {
				if (newx - this.oldx < 0) {
					selection = new Rectangle(newx, this.oldy, -newx + this.oldx,
							newy - this.oldy);
				} else {
					selection = new Rectangle(this.oldx, this.oldy, newx - this.oldx,
							newy - this.oldy);
				}
			}
		}
			...
			if (c.contains(x, y)) {
				...
				if (c instanceof FileObject) {
					...
					if (fo.getLinkedClass() != null
							&& fo.lastClicked() + 500 > System.currentTimeMillis()) {
						try {
							this.createNewWindow(800, 600,
									(Class<? extends AppWindow>) fo.getLinkedClass());
				...
				for (GUIComponent c : aw.guiObjects) {
					if (c instanceof GUIPanel) {
						for (GUIComponent com : ((GUIPanel) c).getGuiObjects()) {
							...
							if (com.contains(x - aw.getX(), y - aw.getY() - 26)) {
								// TODO something is wrong...
								onComponentPressed(button, com);
								break;
							}
						}
		...
		for (int i = windowList.size() - 1; i >= 0; i--) {
			if (windowList.get(i).contains(x, y)) {
				...
				if (success) {
					...
					// TODO FIX THIS
					foregroundWindow(windowList.get(i));
	...
	public void setBackground(Image img) throws SlickException {
		...
		g.drawImage(img.getScaledCopy(background.getWidth(), background.getHeight()), 0,
				0);
		g.flush();
	}
